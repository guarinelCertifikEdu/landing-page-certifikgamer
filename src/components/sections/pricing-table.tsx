...
// NOTE: The following two plans are hidden but kept here for reference.
//       They won't render in the UI because they're not in the exported `plans` array.
/*
const hiddenPlanFree = {
  name: 'Free',
  type: 'free',
  button: {
    text: 'Get started',
...
// Only the Enterprise plan is visible to the user now
  ...
  return (
    <div className="hidden md:grid md:grid-cols-4">
      ...
      {plans.map((plan, index) => (
        <PricingColumn key={plan.name} plan={plan} isHighlighted={false} />
      ))}
    </div>
  );
};
